...
const trendingArtist = () => {
    ...
    return (
        <>
            <div className='flex hover:scale-110 transition md:mx-20 sm:mx-10 mx-5 overflow-x-scroll no-scrollbar space-x-5 '>
                {trendingArtist ?
                    trendingArtist.map((val) => {
                        return (
                            <div onClick={()=>{getArtistInfo(val['artist'])}} className='shrink-0 text-center '>
                                <img src={val['url']} alt="" className='hover:scale-110 transition shadow-xl hover:shadow-indigo-400 rounded-full hover:rounded-3xl mt-2 sm:w-24 sm:h-24 w-24 h-24' />
